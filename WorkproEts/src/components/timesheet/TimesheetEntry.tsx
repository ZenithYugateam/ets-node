...
const TimesheetEntry: React.FC = () => {
  ...
  const fetchTimesheets = async () => {
    try {
      const response = await fetch('https://ets-node-dpa9.onrender.com/api/timesheets');
      if (!response.ok) {
  ...
  const saveEntries = async () => {
    try {
      const response = await fetch('https://ets-node-dpa9.onrender.com/api/timesheets/save_entries', {
        method: 'POST',
