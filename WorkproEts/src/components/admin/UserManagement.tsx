...
const UserManagement = () => {
  ...
  const fetchManagersByDepartment = async (departmentName: string) => {
    try {
      const response = await axios.get(
        `https://ets-node-dpa9.onrender.com/users/managers/${departmentName}`
      );
  ...
  useEffect(() => {
    const fetchAllUsers = async () => {
      try {
        const response = await fetch("https://ets-node-dpa9.onrender.com/api/usersData", {
          method: "POST",
  ...
  useEffect(() => {
    const fetchDepartments = async () => {
      ...
      try {
        const response = await axios.get(
          "https://ets-node-dpa9.onrender.com/api/departments"
        );
  ...
  //     let url = "https://ets-node-dpa9.onrender.com/api/users/add"; // Default URL for POST
  //     let method = "POST"; // Default method

  //     // Only change URL and method if we're updating an existing user
  //     if (selectedUser && selectedUser._id) {
  //       url = `https://ets-node-dpa9.onrender.com/api/users/${selectedUser._id}`;
  //       method = "PUT";
  //     }
  ...
  const handleSave = async () => {
    try {
      ...
      const url = selectedUser?._id
        ? `https://ets-node-dpa9.onrender.com/api/users/edit/${selectedUser._id}`
        : "https://ets-node-dpa9.onrender.com/api/users/add";
    ...
    if (window.confirm("Are you sure you want to delete this user?")) {
      try {
        const response = await fetch(
          `https://ets-node-dpa9.onrender.com/api/users/${userId}`,
          {
            method: "DELETE",
          }
        );
