  ...
  const getData = async () => {
    try {
      const response = await fetch(
        `https://ets-node-dpa9.onrender.com/api/users/${sessionStorage.getItem('userId') || ''}`
      );
      if (!response.ok) {
    ...
    const fetchManagerDetails = async () => {
      ...
      if (storedUserName) {
        ...
        try {
          ...
          const response = await fetch(`https://ets-node-dpa9.onrender.com/api/users/${storedUserId}`);
