...
mongoose.connect("mongodb+srv://codeprabhas121:iZDnnBM35UmVGGNZ@etscluster.7ckqo.mongodb.net/", {
  ...
  app.post('/api/timelog/checkout', async (req, res) => {
    ...
    try {
      ...
      if (reason) {
        timeLog.breaks.push({ start: new Date(), reason });
        await timeLog.save();
        return res.status(200).json({ message: "Break started successfully", timeLog });
      }
